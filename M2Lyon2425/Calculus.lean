...
example : Differentiable ℝ fun x => cos (sin x) * exp x := by
  intros x
  simp only [differentiableAt_sin, DifferentiableAt.cos, differentiableAt_exp, DifferentiableAt.mul]

-- Now see what `hint` has to say!
example : Differentiable ℝ fun x => cos (sin x) * exp x := by
  simp only [differentiable_sin, Differentiable.cos, differentiable_exp, Differentiable.mul]

-- `Function.comp_def` might be useful here
example (x : ℝ) :
    deriv (fun x => cos (sin x) * exp x) x = (cos (sin x) - sin (sin x) * cos x) * exp x := by
  simp only [differentiableAt_sin, DifferentiableAt.cos, differentiableAt_exp, deriv_mul,
    _root_.deriv_cos, Real.deriv_sin, neg_mul, Real.deriv_exp]
  ring
end
...
example : Tendsto (fun  n : ℕ ↦ (n + 1 : ℝ) / n) atTop (𝓝 1) := by
  have h(n : ℝ): (n + 1 : ℝ )/n = 1 + 1/n := by
    rw [div_eq_iff,right_distrib ]
    simp only [one_mul, one_div, add_right_inj]
    sorry
  sorry
...
lemma result1 (a : ℕ → ℝ) (h1 : ∀ n, 0 ≤ a n) (h2 : a ~[atTop] fun n ↦ n) (ε : ℝ) (hε : 0 < ε) :
  ...
  · rw [Metric.tendsto_nhds] at h2
    ...
    specialize h2 ε hε
    filter_upwards [h2,eventually_gt_atTop 0] with n hn
    rw [abs_lt] at hn
    intros hn2
    constructor
    · replace hn := hn.1
      dsimp at hn
      rw[lt_sub_iff_add_lt', lt_div_iff] at hn
      exact hn
      rw[Nat.cast_pos]
      exact hn2
    · sorry
