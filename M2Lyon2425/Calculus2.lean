...
def genBy (A : Set X) : MeasurableSpace X where
  ...
  measurableSet_empty := by
    simp only [Set.top_eq_univ, true_or]
  measurableSet_compl := by
    simp only [Set.top_eq_univ, Set.compl_empty_iff, compl_inj_iff, Set.compl_univ_iff,
      forall_eq_or_imp, Set.compl_empty, or_true, true_or, compl_compl, forall_eq, Set.compl_univ,
      Set.univ_eq_empty_iff, and_self]
...
example : (0 : ℝ≥0∞) * ∞ = 0 := by simp only [zero_mul]

example : (1 : ℝ≥0∞) / 0 = ∞ := by simp only [one_div, ENNReal.inv_zero]
