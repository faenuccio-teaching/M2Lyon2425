...
example [CompactSpace X] {f : X → Y} (hf : Continuous f) :
  ...
  intro ε ε_pos 
  set K := { p : X × X | ε ≤ dist (f p.1) (f p.2)} with hk
...
theorem cauchySeq_of_le_geometric_two' {u : ℕ → X}
  ...
  calc
    ...
    _ ≤ ∑ i in range k, dist (u (N + i)) (u (N + (i + 1))) := by 
      apply dist_le_range_sum_dist (fun x => u (N + x))
...
example [CompleteSpace X] (f : ℕ → Set X) (ho : ∀ n, IsOpen (f n))
  ...
  have : ∀ (n : ℕ) (x : X),
    ...
    by 
    intros n x δ δpos 
    obtain ⟨z, z_in, z_dense⟩ : ∃ z ∈ f n, z ∈ closedBall x δ := by 
      apply hd n x 
      sorry
    sorry
