...
-- This result is false (hence, unprovable)
...
example : ICR ℤ := by -- require some more work
  sorry
...
example (x : ℤˣ) : (x : ℤ) = 1 ∨ (x : ℤ) = -1 := by
  ...
  · left
    exact Int.eq_one_of_mul_eq_one_left hx x.inv_val
  · right
    have hmx : 0 < -(x: ℤ) := by omega
...
noncomputable example {R S : Type*} [Ring R] [Ring S] (f : R →+* S) : Rˣ →* Sˣ where
  toFun := fun x ↦ (map_unit f x x.isUnit).unit
  map_one' := by
...
theorem rZ_transitive : Transitive rZ := by
  intro ⟨x, x'⟩ ⟨y, y'⟩ ⟨z, z'⟩ hxy hyz
  simp only [rZ_iff] at hxy hyz ⊢
  omega
...
instance rZSetoid : Setoid (ℕ × ℕ) := by
  ...
  · exact rZ_reflexive
  · intro x y hxy
    exact rZ_symmetric hxy
  · intro x y z hxy hyz
    exact rZ_transitive hxy hyz
