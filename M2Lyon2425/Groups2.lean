...
def f : MonoidHom₁ (ℕ × ℕ) ℕ where
  toFun p := p.1 * p.2
  map_one := by simp only; simp
  map_mul p q := by simp only; simp; ring
...
lemma map_pow {M N F : Type*} [Monoid M] [Monoid N] [MonoidHomClass₁ F M N] (f : F) (a : M) (n : ℕ) :
  ...
  · simp
    exact map_one f
  · rw[pow_succ,pow_succ,map_mul f, hn]
...
whose inhabitants are `Set` in `G` stable by the group operations.
...
extra effort.
`H:Subgroupe G`, `a∈ H`, then `a∈ H`should make sense.
...
instance : SetLike (Subgroup₁ G) G where
  ...
  coe_injective' := by intro H K; exact Subgroup₁.ext

/- Examples of the coercion to sets un action:-/

example (H : Subgroup₁ G) : 1 ∈ H := H.one_mem   -- elements of a subgroup

example (H : Subgroup₁ G) (α : Type) (f : G → α) := f '' H --ou f '' (H : Set G) au cas où ça déconne -- image by a function
...
instance Subgroup₁Group (H : Subgroup₁ G) : Group H where
  ...
  one_mul x := by
    ...
    --rw[<-SetCoe.ex_iff]
    exact one_mul (x:G)
  mul_one := by intro a; apply SetCoe.ext; exact mul_one (a : G)
  inv := sorry
...
instance : SemilatticeInf (Subgroup₁ G) where
  ...
  inf_le_left H K := by intro x h; exact h.left
  inf_le_right H K := by intro x h; exact h.right
  le_inf := by
    intro a b c h1 h2
    by_contra h3
    sorry
