...
def f : MonoidHom₁ (ℕ × ℕ) ℕ where
  ...
  map_mul := by
    simp only [Prod.fst_mul, Prod.snd_mul, Prod.forall]
    intro a b a1 b1
    rw[mul_assoc ,← mul_assoc a1 b b1,mul_comm a1 b,mul_assoc,mul_assoc]
...
instance : SetLike (Subgroup₁ G) G where
  ...
  coe_injective' f g := by exact Subgroup₁.ext
...
def Subgroup.Setoid (B : Subgroup A) : Setoid A  where
  ...
  iseqv := {
    ...
    symm := by
      ...
      intro h
      have := B.inv_mem h
      have h2 : (x * y⁻¹)⁻¹ = y * x⁻¹ := by group
      rw[h2] at this
      exact this
    trans := by
      intro x y z h1 h2
      have := B.mul_mem h1 h2
      rw[← mul_assoc,mul_assoc x y⁻¹ y,inv_mul_cancel,mul_one] at this
      exact this
  }
...
instance (B : Subgroup A) : Mul (A ⧸ B) where
  mul := Quotient.map₂' (· * ·) (by
    ...
    simp
    change (a*c)*(b*d)⁻¹ ∈ B
    have :(a*c)*(b*d)⁻¹ = a*c *d⁻¹* b⁻¹ := by group
    rw[this]
    have h3 : a * c * d⁻¹ * b⁻¹ = a * b⁻¹ * c * d⁻¹ := by
      rw[mul_comm,← mul_assoc,← mul_assoc,mul_comm b⁻¹ a]
    rw[h3]
    rw[ mul_assoc]
    exact mul_mem h1 h2
...
instance (B : Subgroup A) : CommGroup (A ⧸ B) where
  ...
  one_mul := by
      intro x
      apply Quotient.inductionOn (motive := fun x ↦ 1 * x = x )
      intro a
      change QuotientGroup₁.mk B (1 * a) = QuotientGroup₁.mk B a
      rw[one_mul]
  ...
  mul_one := by
      ...
      intro a
      change QuotientGroup₁.mk B ( a * 1) = QuotientGroup₁.mk B a
      rw[mul_one]
  inv := Inv.inv
  inv_mul_cancel := by
      ...
      intro a
      change QuotientGroup₁.mk B (a⁻¹ * a ) = QuotientGroup₁.mk B 1
      rw[inv_mul_cancel]
  mul_comm := by
      intro x y
      apply Quotient.inductionOn₂ (motive := fun (x : A⧸B) (y : A⧸B) ↦ x * y =y * x )
      intro a b
...
example {G : Type*} [Group G] : MulAction G (Subgroup G) where
  ...
  mul_smul b c  B:= by
    ...
    · intro h1
      ...
      · use h
        exact ⟨ hh.1, rfl⟩
      · have : b * c * h * (b * c)⁻¹ = b * (c * h * c⁻¹) * b⁻¹ := by group
        rw[← this]
        exact hh.2
    · intro h1
      obtain ⟨ h₁ , hh₁⟩ := h1
...
lemma aux_card_eq [Finite G] (h' : Nat.card G = Nat.card H * Nat.card K) :
  ...
  rw[←  Subgroup.index_mul_card H,mul_comm (Nat.card ↥H)   ] at h'
  have : 0 < Nat.card ↥H :=by exact Nat.card_pos
  have h2 := Nat.eq_of_mul_eq_mul_right this h'
  rw[← h2]
...
noncomputable def iso₁ : K ≃* G ⧸ H := by
  set g := QuotientGroup.mk' H
  set f : K →*  G⧸H := MonoidHom.restrict g K
  have h : Function.Bijective f := by
    rw[  Nat.bijective_iff_injective_and_card f]
    constructor
    · rw[← MonoidHom.ker_eq_bot_iff]

      rw[MonoidHom.ker_restrict ]
      simp
      have : g.ker = H  := QuotientGroup.ker_mk' H
      rw[this]
      exact h
    · symm
      exact aux_card_eq h'
...
noncomputable def finalIso : G ≃* H × K := by
  have f : K ≃* G ⧸ H :=  iso₁ h h'
  have f2 := iso₂ h h'
  rw[mul_comm] at h'
  rw[disjoint_comm] at h
  have g : H ≃* G ⧸ K := iso₁ h h'
  have f1 := MulEquiv.prodCongr  g f
  symm

  apply MulEquiv.symm at f2
  have f3  := MulEquiv.trans f1 f2
  exact f3
end
