...
def genBy (A : Set X) : MeasurableSpace X where
  ...
  measurableSet_iUnion := by
    ...
    · obtain ⟨i, hi⟩ := h_top
      ...
      rwa [hi]
    · by_cases h_A : ∃ i, f i = A
      obtain ⟨i, hi⟩ := h_A
      · by_cases h_Ac : ∃ j, f j = Aᶜ
        · obtain ⟨j, hj⟩ := h_Ac
          right; right; right
          rw [eq_top_iff]
          intro x _
        ...
        · have h_eq : ∀ j, f j = A ∨ f j = ∅ := by
          ...
          · intro hx
            ...
            refine ⟨i, ?_⟩
            rwa [hi]
      · by_cases h_Ac : ∃ i, f i = Aᶜ
        · have h_eq : ∀ j, f j = Aᶜ ∨ f j = ∅ := by
...
example : MeasurableSpace X where
  ...
  measurableSet_compl := by
    intro s hs i
    exact (𝓐 i).measurableSet_compl s (hs i)
  measurableSet_iUnion := by
    intro f hf i
    refine (𝓐 i).measurableSet_iUnion _ ?_
    intro j
    exact hf j i
...
example (S T : Set Ω) (hS : μ S ≠ ∞) (hT : MeasurableSet T) :
  ...
  have : S ∩ T ⊆ S := by exact Set.inter_subset_left
  have := OuterMeasureClass.measure_mono μ this
...
-/
example : Measurable (id : Ω → Ω) := by exact fun ⦃t⦄ a ↦ a
...
-- `∫ x in s, f x ∂μ`. -- use `\partial` to write `∂`
...
example
  ...
  · suffices (μ.restrict {ω | 0 < f ω}) (Function.support f) = μ {ω | 0 < f ω} by
      rwa [this]
...
{ sets := { S | μ (Sᶜ) = 0}
  ...
  sets_of_superset := by
    intro S T hS hST
    rw [Set.mem_setOf]
    replace hST : Tᶜ ⊆ Sᶜ := by rwa [Set.compl_subset_compl]
...
example (f g h : Ω → ℝ)
  ...
  filter_upwards [h₁, h₂] with x h1 h2
