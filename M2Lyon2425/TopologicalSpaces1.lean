...
example (s : Set α) : Filter α where
  ...
  inter_sets := by
    intros x y hx hy
    exact subset_inter_iff.2 ⟨hx, hy⟩
...
example : Filter ℕ where
  ...
  univ_sets := by
    dsimp
    use 0
    exact subset_univ (Ici 0)
  sets_of_superset := by
    intros x y hx hy
    obtain ⟨n, hn⟩ := hx
    exact ⟨n, subset_trans hn hy⟩
  inter_sets := by
    intros x y hx hy
    obtain ⟨n, hn⟩ := hx
    obtain ⟨m, hm⟩ := hy
    ...
    have this₁ : Ici (n.max m) ⊆ Ici n := by
      rw [Set.Ici_subset_Ici]
      simp only [le_max_iff, le_refl, true_or]
    have this₂ : Ici (n.max m) ⊆ Ici m := by
      rw [Set.Ici_subset_Ici]
      simp only [le_max_iff, le_refl, true_or]
      right
      trivial
    exact ⟨subset_trans this₁ hn, subset_trans this₂ hm⟩
...
example (a : ℝ) : Filter ℝ where
  ...
  univ_sets := ⟨1, ⟨one_pos, subset_univ (Ioo (a - 1) (a + 1))⟩⟩
  sets_of_superset := by
    intros x y hx hsub
    obtain ⟨ε₁, hε₁⟩ := hx
    exact ⟨ε₁, ⟨hε₁.1, subset_trans hε₁.2 hsub⟩⟩
  inter_sets := by
    intros x y hx hy
    obtain ⟨ε₁, hε₁⟩ := hx
    obtain ⟨ε₂, hε₂⟩ := hy
    refine ⟨min ε₁ ε₂, ?_⟩
    simp only [gt_iff_lt, lt_min_iff, subset_inter_iff]
    refine ⟨⟨hε₁.1, hε₂.1⟩, ?_⟩
    have this₁ : Ioo (a - min ε₁ ε₂) (a + min ε₁ ε₂) ⊆ Ioo (a - ε₁) (a + ε₁) := by
      apply Set.Ioo_subset_Ioo
      apply sub_le_sub
      simp only [le_refl]
      simp only [min_le_iff, le_refl, true_or]
      simp only [add_le_add_iff_left, min_le_iff, le_refl, true_or]
    have this₂ : Ioo (a - min ε₁ ε₂) (a + min ε₁ ε₂) ⊆ Ioo (a - ε₂) (a + ε₂) := by
      apply Set.Ioo_subset_Ioo
      apply sub_le_sub
      simp only [le_refl]
      ...
      right
      trivial
      simp only [add_le_add_iff_left, min_le_iff, le_refl, or_true]
    exact ⟨subset_trans this₁ hε₁.2, subset_trans this₂ hε₂.2⟩

example (a : ℝ) : Filter ℝ where
  sets := {A | ∃ (U : Set ℝ), IsOpen U ∧ a ∈ U ∧ U ⊆ A}
  univ_sets := by
    use univ
    simp only [isOpen_univ, mem_univ, subset_refl, and_self]
  sets_of_superset := by
    intros x y hx hsub
    obtain ⟨U, hU⟩ := hx
    use U
    exact ⟨hU.1, hU.2.1, subset_trans hU.2.2 hsub⟩
  inter_sets := by
    intros x y hx hy
    obtain ⟨U, hU⟩ := hx
    obtain ⟨V, hV⟩ := hy
    use U ∩ V
    exact ⟨IsOpen.inter hU.1 hV.1, ⟨⟨hU.2.1, hV.2.1⟩, Set.inter_subset_inter hU.2.2 hV.2.2⟩⟩
...
def nhds_left (a : ℝ) : Filter ℝ where
  ...
  univ_sets := ⟨1, ⟨one_pos, subset_univ (Ioc (a - 1) a)⟩⟩
  sets_of_superset := by
    intros x y hx hsub
    obtain ⟨ε, hε⟩ := hx
    use ε
    exact ⟨hε.1, subset_trans hε.2 hsub⟩
  inter_sets := by
    intros x y hx hy
    obtain ⟨ε₁, hε₁⟩ := hx
    obtain ⟨ε₂, hε₂⟩ := hy
    ...
    simp only [gt_iff_lt, lt_min_iff, subset_inter_iff]
    refine ⟨⟨hε₁.1, hε₂.1⟩, ?_⟩
    have this₁ : Ioc (a - min ε₁ ε₂) a ⊆ Ioc (a - ε₁) a := by
...
example (s t : Set α) : s ⊆ t ↔
  ...
  · rw [Set.subset_def]
    intros X hX
    exact subset_trans h hX
  · rw [Set.subset_def] at h ⊢
    intros x hx
    specialize h t Set.Subset.rfl
    exact h hx
...
example (F : Filter α) (s : Set α) :
    Filter.principal s ≤ F ↔ ∀ A ∈ F, s ⊆ A := Filter.le_def

#print Filter
example (F : Filter α) (s : Set α) :
    F ≤ Filter.principal s ↔ s ∈ F := by
  rw [Filter.le_def]
  exact ⟨fun h ↦ h s Set.Subset.rfl, fun h x hx ↦ sets_of_superset F h hx⟩
...
example {s : Set α} (f : α → β) :
  ...
  ext X
  refine ⟨fun h ↦ ?_, fun h ↦ ?_⟩
  · rw [map, Filter.mem_mk, Set.mem_preimage, Filter.mem_sets, Filter.principal, Filter.mem_mk] at h
    dsimp at h
    rw [← Set.image_subset_iff] at h
    rw [Filter.principal, Filter.mem_mk]
    dsimp
    exact h
  · sorry
...
-- Use Filter.map_le_iff_le_comp to prove the statement below
example {X Y : Type*} (f : X → Y) (F : Filter X) (G : Filter Y) :
...
example (f : ℝ → ℝ) (a b : ℝ) :
  ...
  exact Filter.HasBasis.tendsto_iff (nhds_basis_Ioo_pos a) (nhds_basis_Ioo_pos b)

#check nhds_basis_Ioo_pos
...
example : ∃ᶠn in atTop (α := ℕ), Prime n := sorry
