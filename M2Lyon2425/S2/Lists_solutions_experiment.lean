...
inductive isNice : ℕ → Prop
 | of_zero : isNice 0
 | of_five_mul (n : ℕ) : isNice n → isNice (n + 5)

example (n : ℕ) : ¬ isNice (n^2+2) := by
  intro h
  have : ∀ m, n^2+2 ≠ m + 5 := sorry
  simp [this]
...
#synth DecidableEq (List Stations)
...
-- instance : Decidable (fun D :

instance : Fintype (Directions) := by
  fconstructor
  · use {A_NS, A_SN, B_SN, B_NS, C_SN, C_NS, D_EW, D_WE}
    simp
  · simp only [Multiset.insert_eq_cons, Finset.mk_cons, Finset.cons_eq_insert, Finset.mem_insert,
    Finset.mem_mk, Multiset.mem_singleton, Subtype.forall, Subtype.mk.injEq, reverse_cons,
    reverse_nil, nil_append, singleton_append, cons_append]
    intro L hL
    -- cases hL-- with _ | _ | _ | _ | ⟨M, hM⟩
    rcases hL with _ | _ | _ | _ | h
    · simp_all
    · simp_all
    · simp_all
    · simp_all
    · rw [reverse_eq_iff, reverse_eq_iff, reverse_eq_iff, reverse_eq_iff]
      cases h
  ...
  --   cases D.1
  --   simp only [Multiset.insert_eq_cons, Finset.mk_cons, Finset.cons_eq_insert, Finset.mem_insert,
  --     Subtype.mk.injEq, reverse_cons, reverse_nil, nil_append, singleton_append, cons_append,
...
structure Trip (start arrival : Stations) where
 .1 : List Stations
  not_empty :.1 ≠ []
  start :.1.head not_empty = start
  arrival :.1.getLast not_empty = arrival
  -- nodup :.1.Nodup
  connection (l : List Stations) : IsInfix l.1 → l.length = 2 →
    ∃ D : Directions, IsInfix l D.1
...
lemma Connected_trans {pt₁ pt₂ pt₃} (h12 : Connected pt₁ pt₂) (h23 : Connected pt₂ pt₃) :
  ...
  · rw [getLast_append_of_ne_nil t23.not_empty]
    exact t23.arrival
  · intro l hl hltwo
    by_cases in_12 : IsInfix l t12.1
    · sorry
    · by_cases in_23 : IsInfix l t23.1
      · sorry
      · sorry
