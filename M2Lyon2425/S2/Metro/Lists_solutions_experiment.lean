...
lemma isTrip_symm' {L : List Stations} (hL : IsTrip L) : IsTrip L.reverse := by
  have h_neL := ne_nil_Trip hL
  obtain ⟨s, M, hM⟩ := exists_cons_of_ne_nil <| ne_nil_Trip hL
  by_cases h_ne : M = []
  · rw [h_ne] at hM
    rw [hM]
    simpa using IsTrip.no_move s _
  · rw [hM]
    rw [reverse_cons]
    replace h_ne : ¬ M.reverse = [] := sorry
    have taut : M.reverse = (M.reverse.head h_ne :: M.reverse.tail) := by
      sorry
    rw [taut]
    have : (M.reverse.tail ++ [s]).length < L.length := sorry
    apply cons_isTrip
    · sorry
    simp
