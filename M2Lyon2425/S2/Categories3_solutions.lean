...
def TwoMor.isoMk {X Y : Span C} {a b : X ⟶ Y}
    ...
    (comp₂ : u.hom ≫ b.right = a.right) : a ≅ b where
      ...
      inv := by
        ...
        · rw [← cancel_epi u.hom, ← assoc, Iso.hom_inv_id,
          id_comp, comp₁]
        · rw [← cancel_epi u.hom, ← assoc, Iso.hom_inv_id,
          id_comp, comp₂]
...
@[simp]
noncomputable def SpanWhiskerLeft {X Y Z : Span C} (a : X ⟶ Y)
    {b b' : Y ⟶ Z} (u : b ⟶ b') : a ≫ b ⟶ a ≫ b' where
      hom_roof := pullback.lift (pullback.fst _ _)
        (pullback.snd _ _ ≫ u.hom_roof)
        (by simp [pullback.condition])

@[simp]
noncomputable def SpanWhiskerRight {X Y Z : Span C} {a a' : X ⟶ Y}
    (u : a ⟶ a') (b : Y ⟶ Z) : a ≫ b ⟶ a' ≫ b where
      hom_roof := pullback.lift (pullback.fst _ _ ≫ u.hom_roof)
        (pullback.snd _ _) (by simp [pullback.condition])

@[simp]
noncomputable def SpanAssociator {X Y Z T : Span C} (a : X ⟶ Y)
    (b : Y ⟶ Z) (c : Z ⟶ T) : (a ≫ b) ≫ c ≅ a ≫ (b ≫ c) :=
  TwoMor.isoMk (pullbackAssoc _ _ _ _) (by simp) (by simp)

@[simp]
noncomputable def SpanLeftUnitor {X Y : Span C} (a : X ⟶ Y) :
    (𝟙 X) ≫ a ≅ a := by
...
@[simp]
noncomputable def SpanRightUnitor {X Y : Span C} (a : X ⟶ Y) :
    a ≫ (𝟙 Y) ≅ a := by
  refine TwoMor.isoMk ?_ ?_ ?_
  · exact asIso (pullback.fst a.right (𝟙 Y.pt))
  · simp
  · simp [pullback.condition]
...
noncomputable instance : Bicategory (Span C) where
  ...
  whiskerLeft := SpanWhiskerLeft
  whiskerRight := SpanWhiskerRight
  associator := SpanAssociator
  leftUnitor := SpanLeftUnitor
  rightUnitor := SpanRightUnitor
  id_whiskerLeft _ := by
    ext
    simp only [instCategoryStructSpan, OneMorComp, OneMorId,
      TwoMor.isoMk, TwoMor.homMk, asIso_hom, asIso_inv,
      TwoMor.comp]
    ext <;> simp [← pullback.condition]
  whiskerRight_id _ := by
    ext
    simp only [instCategoryStructSpan, OneMorComp, OneMorId,
      TwoMor.isoMk, TwoMor.homMk, asIso_hom, asIso_inv,
      TwoMor.comp]
...
noncomputable def ToSpan :
  ...
  mapComp {X Y Z} f g := by
    ...
    · simp only [LocallyDiscrete.comp_as, Iso.symm_hom, asIso_inv,
      ...
      simp
-- `aesop_cat` solves all the remaining goals.
