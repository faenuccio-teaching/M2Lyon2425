...
example (hp : ∀ M : Type, [AddCommGroup M] → [NormedModuleBad M] → ∀ m : M, p (rel m)) (M: Type) [AddCommGroup M][NormedModuleBad M] (f : M → M) : p (rel f) := by
  specialize hp (M → M) f
  sorry --exact hp
...
example (hp : ∀ M : Type, [AddCommGroup M] → [NormedModuleGood M] → ∀ m : M, p (rel m)) (M: Type) [AddCommGroup M][NormedModuleGood M] (f : M → M) : p (rel f) := by
  specialize hp (M→M) f
  exact hp
  sorry
