...
/- First goal: the functor `G` has a left adjoint if and only if, for
every `X : C`, the comma category ` X ↓ G` has an initial object.
"Remember" that `X ↓ G` is the category whose objects are pairs
`(Y,f)` with `Y : D` and `f : X ⟶ G.obj Y`, and whose morphisms are
commutative diagrams.

The category `X ↓ G` is a special case of `CategoryTheory.Comma F G`
where `F` is the functor from the one-point category `PUnit` to `C` sending
the unique object to `X`.
It already exists in current mathlib and is called `StructuredArrow X G`.
-/
...
/-
If `A : StructuredArrow X G`, its fields are called `A.right` (the object
of `D`) and `A.hom` (the morphism `X ⟶ G.obj A.right`).
-/

/-
First direction: if `G` has a left adjoint, then each category
`StructuredArrow X G` has an initial object. We will use
`Limits.hasInitial_of_unique` to construct the `HasInitial` classes.
This requires us to find the initial object first.
-/
...
the category `StructuredArrow X G` has an initial object, which is
given by `adj.unit.app X : X ⟶ G.obj (F.obj X)`.
-/
def HasInitialOfLeftAdjoint {F : C ⥤ D} (adj : F ⊣ G) (X : C) :
    HasInitial (StructuredArrow X G) := by
  set I : StructuredArrow X G := StructuredArrow.mk (adj.unit.app X)
...
/- The other direction: if each category `X ↓ G` has an initial object,
then `G` has a left adjoint `F`. We will define `F.obj X` as the
`right` component of the initial object of `StructuredArrow X G`,
and the `hom` component will give us the unit of the adjunction at `X`.

Note that, if `u` is a morphism in `StructuredArrow X G`, then it has
fields `u.right` (the underlying morphism of objects of `D`) and
`u.w` (the commutative triangle).

To check the requisite compatibilities, we use many times the
fact that, if `X` is initial, then any two morphisms `f, g : X ⟶ Y`
are equal. This is called `Limits.initial.hom_ext`.
-/

#check Limits.initial
#check Limits.initial.to
#check StructuredArrow.map -- functor from `StructuredArrow X' G` to
-- `StructuredArrow X G` induced by a morphism `f : X ⟶ X'`
...
noncomputable def UnitOfInitial : 𝟭 C ⟶ FunctorOfInitial G h ⋙ G where
      app X := (initial (StructuredArrow X G)).hom
      naturality X X' f := sorry

noncomputable def CounitOfInitial : G ⋙ FunctorOfInitial G h ⟶ 𝟭 D where
      app Y := sorry
...
noncomputable def AdjunctionOfInitial : FunctorOfInitial G h ⊣ G :=
  Adjunction.mkOfUnitCounit (Adjunction.CoreUnitCounit.mk (UnitOfInitial G h)
  (CounitOfInitial G h) (LeftTriangleOfInitial G h) (RightTriangleOfInitial G h))

include h in
lemma IsRightAdjointOfInitial : G.IsRightAdjoint where
  exists_leftAdjoint :=
    ⟨FunctorOfInitial G h, Nonempty.intro (AdjunctionOfInitial G h)⟩
