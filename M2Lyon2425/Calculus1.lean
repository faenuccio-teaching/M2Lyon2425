...
example : Differentiable ℝ fun x => cos (sin x) * exp x := by
  ...
  · change DifferentiableAt ℝ (cos ∘ sin) x
...
example : Differentiable ℝ fun x => cos (sin x) * exp x := sorry
...
example (x : ℝ) :
  ...
  · simp only [Real.deriv_exp]
    rw [← Function.comp_def, deriv.comp]
    · simp only [deriv_cos', Real.deriv_sin, neg_mul]
      ring
    · exact differentiableAt_cos
    · exact differentiableAt_sin
...
example : Tendsto (fun  n : ℕ ↦ (n + 1 : ℝ) / n) atTop (𝓝 1) := by
  have h₁ := tendsto_const_div_atTop_nhds_zero_nat 1
  have h₂ : Tendsto (fun n : ℕ ↦ (1 : ℝ)) atTop (𝓝 1) := tendsto_const_nhds
  have h₃ := Tendsto.add h₁ h₂
  rw [zero_add] at h₃
...
example : Tendsto (fun x ↦ (exp x - 1) / (sin x)) (𝓝[≠] 0) (𝓝 1) := by
  ...
  · filter_upwards
    intro a
    simp only [differentiableAt_const, DifferentiableAt.sub_iff_left]
    exact differentiableAt_exp
  · sorry
  · sorry
  · sorry
...
lemma result1 (a : ℕ → ℝ) (h1 : ∀ n, 0 ≤ a n) (h2 : a ~[atTop] fun n ↦ n) (ε : ℝ) (hε : 0 < ε) :
  ...
  · rw [Metric.tendsto_nhds] at h2
    ...
    specialize h2 ε hε
    filter_upwards [h2, eventually_ne_atTop 0] with n hn hn'
    rw [abs_lt] at hn
    constructor
    · replace hn := hn.1
      dsimp at hn
      rw [← lt_div_iff, sub_eq_add_neg, ← lt_sub_iff_add_lt']
      exact hn
      simp only [Nat.cast_pos]
      exact Nat.zero_lt_of_ne_zero hn'
    · simp only [Pi.div_apply, neg_lt_sub_iff_lt_add] at hn
      have := hn.2
      sorry
  · sorry
...
lemma result2 {s : ℝ} (hs : 1 < s) : Summable (fun n ↦ a n ^ (- s)) := by
  ...
  rw [norm_eq_abs, norm_eq_abs, one_add_one_eq_two, ← abs_two, ← abs_mul]
  refine abs_le_abs ?h.h.h₀ ?h.h.h₁
  replace hn := hn.2
  rw [one_add_one_eq_two] at hn
  sorry
  ...
  have h_single : ∀ n, Tendsto (fun s : ℝ ↦ (s - 1) * v n ^ (- s)) (𝓝[>] 1) (𝓝 0) := by
    ...
    have lim2 : Tendsto (fun s : ℝ ↦ v n ^ (- s)) (𝓝 1) (𝓝 ((v n) ⁻¹)) := by
      ...
      · simp_rw [Real.rpow_neg (hv _)]
        ...
        have := Real.continuousAt_const_rpow' (a := v n) (b := 1)
        specialize this one_ne_zero
        have := this.tendsto
